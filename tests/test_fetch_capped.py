...
from src.main import MAX_BACKGROUND_BYTES, fetch_capped  # noqa: E402
import requests
...
def test_returns_body_under_cap():
    with mock.patch("src.main.requests.get", return_value=fake_response([b"ab", b"cd"])):
        assert fetch_capped("http://x") == b"abcd"


def test_drops_body_over_cap():
    oversized = [b"x" * (MAX_BACKGROUND_BYTES + 1)]
    with mock.patch("src.main.requests.get", return_value=fake_response(oversized)):
        assert fetch_capped("http://x") is None


def test_returns_none_on_request_failure():
    with mock.patch("src.main.requests.get", side_effect=requests.RequestException):
        assert fetch_capped("http://x") is None


def test_returns_none_on_error_status():
    bad = fake_response([], raises=requests.HTTPError)
    with mock.patch("src.main.requests.get", return_value=bad):
        assert fetch_capped("http://x") is None
